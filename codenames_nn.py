...
def create_shallow_model(img_shape, num_classes, activation_fn):
  ...
  x = Dense(10, activation=activation_fn, name='fc1')(x)
  x = Dense(10, activation=activation_fn, name='fc2')(x)
...
def main(_):
  ...
  if (FLAGS.use_pretrained and os.path.exists(FLAGS.model_name + ".json")):
    ...
    hist = model.fit(x_train, y_train, epochs=FLAGS.num_epochs, batch_size=FLAGS.minibatch_size, validation_data=(x_dev, y_dev), verbose=1)
